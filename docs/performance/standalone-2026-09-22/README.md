...
| --- | ---: | ---: | ---: |
| One active task, 200 events | 57.97 ms | 801.46 ms | 981.97 ms |
| Ten concurrent tasks, 200 events | 77.71 ms | 872.58 ms | 971.38 ms |
...
- Full attempt 3 at `e7136a9c6cb73a059c71afea06d870477c98a5aa`: both preparation and measurement cleanup passed. The dashboard observed disconnection, opened a second feed, and received post-outage revision 473 in 796.38 ms. All unchanged targets and scenarios passed. This passed the review corrections; subsequent CI exposed a missing npm-path assumption on hosted runners.

- Full attempt 4 at the measured revision above: the launcher explicitly mounts the selected npm distribution, which repairs the hosted-runner assumption. All targets/scenarios and both preparation/measurement cleanup pass. Dashboard recovery took 797.65 ms with a second feed and revision 473. This is the final acceptance run. No unchanged run was repeated to select more favorable timings.
