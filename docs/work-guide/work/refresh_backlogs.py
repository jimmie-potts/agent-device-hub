...
def refresh_repo(repo):
    ...
    if repo == 'agent-device-hub':
        for number in (3, 4, 6, 7, 8, 9, 13, 49, 54, 80, 100, 103, 107):
            baseline = normalize(api(f'{base}/issues/{number}'))
            assert baseline['state'] == 'CLOSED'
            issues.append(baseline)
            direct.append({'number': number, 'state': baseline['state'], 'purpose': 'Closed guide reference' if number in (6, 8, 9, 13, 49, 54, 80, 100, 103, 107) else 'Completed state baseline' if number == 3 else 'Completed contract/MCP baseline'})
