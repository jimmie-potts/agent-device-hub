...
def refresh_repo(repo):
    ...
    targets = {'agent-device-hub': [2, 5, 30, 50, 83, 91], 'codex-nanoleaf': [26, 29, 30, 34, 37, 49, 52, 53, 54, 55], 'divoom-app-upgrade': [12, 26, 29, 30, 31, 32, 33, 37, 46]}[repo]
    direct = []
    ...
    if repo == 'agent-device-hub':
        for number in (3, 4, 6, 7, 8, 13, 49, 54, 63, 80, 100, 103, 107):
            baseline = normalize(api(f'{base}/issues/{number}'))
            assert baseline['state'] == 'CLOSED'
            issues.append(baseline)
            direct.append({'number': number, 'state': baseline['state'], 'purpose': 'Closed guide reference' if number in (6, 8, 13, 49, 54, 63, 80, 100, 103, 107) else 'Completed state baseline' if number == 3 else 'Completed contract/MCP baseline'})
