...
def refresh_repo(repo):
    ...
    targets = {'agent-device-hub': [2, 5, 50, 83, 91], 'codex-nanoleaf': [26, 29, 34, 37, 49, 52, 53, 54, 55], 'divoom-app-upgrade': [12, 26, 29, 30, 31, 32, 33, 37, 46]}[repo]
    direct = []
    for number in targets:
        ...
        references_only = (repo == 'codex-nanoleaf' and number in (54, 55)) or (repo == 'divoom-app-upgrade' and number == 30)
    ...
    if repo == 'agent-device-hub':
        for number in (3, 4, 7, 13, 49, 54, 80, 100, 103, 107):
            ...
            direct.append({'number': number, 'state': baseline['state'], 'purpose': 'Closed guide reference' if number in (13, 49, 54, 80, 100, 103, 107) else 'Completed state baseline' if number == 3 else 'Completed contract/MCP baseline'})
