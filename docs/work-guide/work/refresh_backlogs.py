...
def refresh_repo(repo):
    ...
    if repo == 'agent-device-hub':
        for number in (4, 7, 54, 100):
            baseline = normalize(api(f'{base}/issues/{number}'))
            assert baseline['state'] == 'CLOSED'
            issues.append(baseline)
            direct.append({'number': number, 'state': baseline['state'], 'purpose': 'Closed guide reference' if number in (54, 100) else 'Completed contract/MCP baseline'})
