...
def refresh_repo(repo):
    ...
    if repo == 'agent-device-hub':
        for number in (3, 4, 7, 54, 100, 107):
            baseline = normalize(api(f'{base}/issues/{number}'))
            assert baseline['state'] == 'CLOSED'
            issues.append(baseline)
            direct.append({'number': number, 'state': baseline['state'], 'purpose': 'Closed guide reference' if number in (54, 100, 107) else 'Completed state baseline' if number == 3 else 'Completed contract/MCP baseline'})
