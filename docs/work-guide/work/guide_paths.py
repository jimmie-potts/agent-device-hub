...
TOPICS = [
    ...
    ('bunny-controls', 'B.U.N.N.Y. interface and device controls',
     ...
     "B.U.N.N.Y. is the one shell (ADR 0007). The Neon skin, one-step apply, bounded browser sessions and one command lifecycle are delivered. Bookmark access, shared Prism device art, controller setup documentation and controls for the Panels' read-only snapshot are delivered in source. The dense home widget grid, gesture-driven controls and hash routes are also delivered in source. Next: extend the home widgets and device pages, fix component aliases that collide with built-in pages, and build the group page.", ['H287', 'H286', 'H247', 'H271']),
    ('nanoleaf-presentation', 'Nanoleaf wall map and effects',
     ...
     "Codex task titles, current projects first and the compact inspector are delivered. Thread links, the simpler default layout, the Open in Codex focus fix, the merged inspector card and configurable Work-mode colors are delivered. Next: show the Lines hold, keep comets on skipped revisions and queue the delayed completion comet. Claude Code titles wait on three open decisions. Switching between Work and Free by session presence no longer waits on session retirement, but it keeps a blocked label. Saved-scene playback and requested animations through MCP are delivered; stopping an animation and restoring the remembered scene through MCP is delivered in source. The wall map stays Nanoleaf's advanced editor (ADR 0007), so new editing controls land in B.U.N.N.Y.", ['N112', 'N115', 'N81', 'N150']),
...
DEVICE_TRACKS = {
    ...
    'Tidbyt': ['H227', 'H23', 'H24'],
    'LIFX': ['H22', 'H319', 'H320', 'H327', 'H337', 'H416', 'H439'],
