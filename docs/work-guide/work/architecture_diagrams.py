...
MAP_DETAILS = {
    ...
    'future': dict(role='Shared playback runs in the hub with the Sony HT-A9 as its installed source (#175), and now-playing with Sony-qualified controls is delivered (#37). Hub moments are decided (ADR 0006): rules and the interrupt set (#358) and the moment sender are planned, and controller contract 1.1 carries the moment command. The Windows music connector (#36) is deferred and would run as a PC-side relay under ADR 0008. Rules may combine playback and task facts but never bypass a controller queue.', sources=[(H, 'docs/architecture.md'), (H, 'docs/iphone-apple-music-qualification.md')]),
    ...
    'newCtl': dict(role='The Tidbyt cloud controller (64×32) and the LIFX LAN controller are in-process TypeScript packages under controllers/, served to the hub by one installed local controller host (#289) with one writer per device. Tidbyt agent status and the now-playing card are installed; LIFX automatic status waits on the owner\'s behavior decisions (#20). PC lighting remains in the backlog.', sources=[(H, 'controllers/tidbyt/README.md'), (H, 'docs/architecture.md')]),
...
DIAGRAMS = [
    ...
    dict(id='arch-shared-system', spec=D2, kind='architecture', status='implemented',
         ...
         reading=['Green arrows are the observation path: providers → hooks → hub routes → core → private store. Dashed arrows are the state feed that consumers read; red arrows are explicit controller v1 commands from the dashboard or MCP clients; gray arrows are internal calls, commits and the library import. Publication and commands never share a path.',
                  ...
                  'Tags record source state at the pinned revision. Installed appears only where an owning issue recorded installed acceptance ([[N55]], [[N30]]); the hub runs as the installed shared monitor service ([[N30]]) beside the installed local controller host ([[H289]]).'],
         ...
         issues=['H2', 'H3', 'H5', 'H6', 'H13', 'P31', 'N29', 'N55', 'H36', 'H175', 'H37', 'H291', 'H358', 'H289', 'H53']),
    dict(id='arch-nanoleaf-linux', spec=D9, kind='architecture', status='implemented', revisions=VIEW,
         ...
         sources=[(H, 'docs/architecture.md'), (H, 'docs/decisions/0008-runtime-hosting.md'), (N, 'docs/decisions/0007-linux-runtime-ownership.md'), (N, 'docs/decisions/0010-per-device-worker-and-nl22.md'), (N, 'docs/decisions/0012-retire-windows-runtime.md'), (N, 'docs/linux-install.md'), (N, 'docs/shared-input.md'), (N, 'bridge/install_linux.py'), (N, 'bridge/devices.py'), (N, 'bridge/wall_server.py'), (N, 'bridge/controller_server.py')],
         issues=['H43', 'N54', 'N55', 'N30', 'N46', 'N89', 'N131', 'N132', 'H356']),
    ...
    dict(id='seq-owner-migration', spec=D8, kind='sequence', status='implemented', revisions=VIEW,
         ...
         boundaries=['Released [[H3]] defines export/import format 1.0. [[P31]] implements embedded storage, quiesce/export/import and the remote facade. [[H5]] delivered supervised release, fenced import and rollback after writes; [[H8]] added Linux/WSL setup and Nanoleaf cutover.',
                     ...
                     'Controller databases stay private; Windows and WSL never coordinate through a mounted SQLite file. Repository moves ([[H25]], [[H26]]) are separate changes; under ADR 0008 the runtime stays in WSL with a keep-alive ([[H356]]) and Docker packaging belongs to the dedicated server ([[H44]]).'],
