...
D2 = arch(
    ...
    components=[
        ...
        c2('future', 'external', 'Music + moments', 'HT-A9 source · rules planned', 0, 4, 'playback installed'),
        ...
        c2('newCtl', 'cloud', 'New controllers', 'Tidbyt · LIFX · PC lighting', 3, 2, 'host installed · PC later'),
    ...
    boundaries=[
        {'kind': 'region', 'label': 'Linux hub process (apps/hub) · installed shared monitor', 'wraps': ['mcpModule', 'hubRoutes', 'core', 'hubStore']},
...
MAP_DETAILS = {
    ...
    'future': dict(role='Shared playback runs in the hub with the Sony HT-A9 as its installed source (#175), and now-playing with Sony-qualified controls is installed and checked (#37). Hub moments are decided (ADR 0006): rules and the interrupt set (#358) and the moment sender are planned, and controller contract 1.1 carries the moment command. The Windows music connector (#36) is deferred; it would stay PC-local, or run through the PC-side relay ADR 0008 proposes once the runtime leaves the PC (#44). Rules may combine playback and task facts but never bypass a controller queue. These statements rest on the linked issues\' acceptance records and on the post-baseline sources recorded in the architecture source receipts, not on the baseline-pinned files.', sources=[(H, 'docs/architecture.md'), (H, 'docs/iphone-apple-music-qualification.md')]),
    ...
    'nanoStore': dict(role='Linux-owned SQLite under the Nanoleaf data directory holds tasks, projects, wall preferences and controller state. It stays on Linux; the Windows runtime was retired from source and decommissioned (Nanoleaf ADR 0012), and no runtime database is shared through /mnt/c.', sources=[(N, 'docs/linux-install.md'), (N, 'docs/decisions/0007-linux-runtime-ownership.md')]),
    ...
    'newCtl': dict(role='The Tidbyt cloud controller (64×32) and the LIFX LAN controller are in-process TypeScript packages under controllers/, served to the hub by one installed local controller host (#289) with one writer per device. Tidbyt agent status and the now-playing card are installed; LIFX automatic status waits on the owner\'s behavior decisions (#20). PC lighting remains in the backlog. The installed state rests on #289 and #21, which postdate the baseline pin.', sources=[(H, 'controllers/tidbyt/README.md'), (H, 'docs/architecture.md')]),
...
D7 = seq(
    ...
    cards=[
        ...
        {'dot': 'amber', 'title': 'Prerequisites', 'items': ['H67 needs H63, H32, H31, H5, Nanoleaf #49 and Pixoo #33', 'H68 needs H65 and H67; H69 also needs H8', 'Manual dispatch does not need automation engine H45; Music H71 follows H40, H68 and H175']},
    ],
    width=1180, segment_gap=30,
)
...
D9 = arch(
    ...
    connections=[
        ...
        {'id': 'desktop-hooks', 'from': 'desktop', 'to': 'hooksCli', 'label': 'hooks on rollback', 'variant': 'dashed', 'fromSide': 'bottom', 'toSide': 'top', 'labelDx': -30, 'labelDy': 24},
        {'id': 'cli-hooks', 'from': 'cliClient', 'to': 'hooksCli', 'label': 'hooks on rollback', 'variant': 'dashed', 'fromSide': 'left', 'toSide': 'right', 'labelDy': -18},
        {'id': 'cli-mcp', 'from': 'cliClient', 'to': 'mcpHost', 'label': 'MCP', 'variant': 'emphasis', 'fromSide': 'bottom', 'toSide': 'top', 'labelDx': 24, 'labelDy': 24},
        {'id': 'hooks-state', 'from': 'hooksCli', 'to': 'linuxState', 'label': 'CLI · rollback events', 'variant': 'dashed', 'fromSide': 'bottom', 'toSide': 'top', 'labelDx': 40, 'labelDy': 24},
    ...
    cards=[
        {'dot': 'violet', 'title': 'Processes and state', 'items': ['Three user services: wall map, controller and MCP host', 'Workers start on demand, one instance and lock per device', 'CLI, map and controller share the private Linux SQLite']},
...
DIAGRAMS = [
    dict(id='arch-local-paths', spec=D1, kind='architecture', status='implemented', revisions=VIEW,
         ...
         boundaries=['Pixoo MCP tools keep their string request_id. The optional /controller/v1 boundary in [[P37]] shares the same ledger, Player and writer, with scoped bearer authentication and guarded native envelopes.',
                     ...
                     'Historical: before the Linux runtime ([[N54]], [[N55]]), this route forwarded to a Windows controller API, directly on Windows or through a WSL helper. Nanoleaf ADR 0012 removed that route from source ([[N131]]) and the Windows installation was decommissioned ([[N132]]); Git history keeps it. No route opens SQLite.'],
         sources=[(H, 'packages/mcp/README.md'), (P, 'docs/local-mcp.md'), (P, 'apps/server/src/mcp.ts'), (P, 'apps/server/src/control-service.ts'), (P, 'packages/device/src/http-adapter.ts'), (P, 'docs/hub-controller-api.md'), (N, 'docs/local-mcp.md'), (N, 'mcp/src/server.ts'), (N, 'mcp/src/transport.ts'), (N, 'docs/controller-api.md'), (N, 'docs/decisions/0010-per-device-worker-and-nl22.md'), (N, 'docs/decisions/0012-retire-windows-runtime.md')],
         issues=['P37', 'N34', 'N55', 'N64', 'N131', 'N132']),
    dict(id='arch-shared-system', spec=D2, kind='architecture', status='implemented',
         status_label='Source delivered at the pinned revisions; installed and planned nodes are tagged', short='System map',
         summary='Where B.U.N.N.Y. runs at the pinned revisions. Provider hooks send lifecycle metadata to the Linux hub process, which composes the agent-state core, a private SQLite store, the authenticated HTTP routes and the embedded MCP module. Each controller keeps its own designated writer and private state. Playback and the local controller host are labelled installed; rules and PC lighting are labelled planned.',
         reading=['Green arrows are the observation path: providers → hooks → hub routes → core → private store. Dashed arrows are the state feed that consumers read; red arrows are explicit controller v1 commands from the dashboard or MCP clients; gray arrows are internal calls, commits and the library import. Publication and commands never share a path.',
                  ...
                  'Tags record state at the pinned revision or, where amended, at the linked issue\'s acceptance record. Installed appears only where an owning issue recorded installed acceptance ([[N55]], [[N30]], [[H175]], [[H289]]); the hub runs as the installed shared monitor service ([[N30]]) beside the installed local controller host ([[H289]]).'],
         boundaries=['Hub source is pinned at the revision listed below the map; Nanoleaf and Pixoo keep their earlier pins. Source delivery is not installation, transport or physical acceptance.',
                     ...
                     'Shared playback with the Sony source is installed ([[H175]]) and now-playing is installed and checked ([[H37]]); hub moments are decided ([[H291]]) with rules ([[H358]]) planned, and the Windows music connector ([[H36]]) is deferred. Tidbyt and LIFX run in the installed local controller host ([[H289]]); PC lighting ([[H53]]) remains planned. The map labels them without implying physical acceptance.'],
         details=MAP_DETAILS,
         ...
         issues=['H2', 'H3', 'H5', 'H6', 'H13', 'P31', 'N29', 'N55', 'N30', 'H36', 'H175', 'H37', 'H291', 'H358', 'H289', 'H53']),
    dict(id='arch-nanoleaf-linux', spec=D9, kind='architecture', status='implemented', revisions=VIEW,
         ...
         boundaries=['Installed acceptance of the Linux runtime is recorded in [[N55]], and of shared input in [[N30]]; [[H43]] adopted the runtime in hub architecture. The Windows runtime was retired from source ([[N131]]) and decommissioned on the PC ([[N132]]). Availability follows WSL until the keep-alive of ADR 0008 ([[H356]]) is installed.',
                     ...
                     'Under hub ADR 0007 the wall map service is Nanoleaf\'s advanced editor: B.U.N.N.Y. is the one shell, and the wall map retires once each of its operations has a shell home ([[N171]]).'],
         sources=[(H, 'docs/architecture.md'), (H, 'docs/decisions/0007-bunny-shell.md'), (H, 'docs/decisions/0008-runtime-hosting.md'), (N, 'docs/decisions/0007-linux-runtime-ownership.md'), (N, 'docs/decisions/0010-per-device-worker-and-nl22.md'), (N, 'docs/decisions/0012-retire-windows-runtime.md'), (N, 'docs/linux-install.md'), (N, 'docs/shared-input.md'), (N, 'bridge/install_linux.py'), (N, 'bridge/devices.py'), (N, 'bridge/wall_server.py'), (N, 'bridge/controller_server.py')],
         issues=['H43', 'N54', 'N55', 'N30', 'N46', 'N89', 'N131', 'N132', 'H356', 'N171']),
    ...
    dict(id='seq-owner-migration', spec=D8, kind='sequence', status='implemented', revisions=VIEW,
         ...
         boundaries=['Released [[H3]] defines export/import format 1.0. [[P31]] implements embedded storage, quiesce/export/import and the remote facade. [[H5]] delivered supervised release, fenced import and rollback after writes; [[H8]] added Linux/WSL setup and Nanoleaf cutover.',
                     ...
                     'Controller databases stay private; Windows and WSL never coordinate through a mounted SQLite file. Repository moves ([[H25]], [[H26]]) are separate changes; under ADR 0008 the runtime stays in WSL until its keep-alive ([[H356]]) is installed, and Docker packaging belongs to the dedicated server ([[H44]]).'],
         ...
         sources=[(H, 'apps/hub/SETUP.md'), (H, 'apps/hub/README.md'), (H, 'apps/hub/src/migration.ts'), (H, 'apps/hub/src/migration-routes.ts'), (H, 'docs/architecture.md'), (H, 'docs/decisions/0008-runtime-hosting.md'), (H, 'packages/agent-state/README.md'), (P, 'docs/agent-monitoring.md'), (P, 'apps/server/src/monitor-source.ts'), (N, 'docs/shared-input.md')],
         issues=['H5', 'H8', 'P31', 'P34', 'N29', 'N30', 'H356', 'H44']),
