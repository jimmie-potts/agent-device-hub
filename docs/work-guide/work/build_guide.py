...
GUIDES = [
    ...
    dict(id='shared-codex', short='Shared Codex integration', title='Deliver shared Codex monitoring and integration controls', phase='Main development path',
         ...
         headers=['Stage', 'Owning issues', 'Result'], rows=[
             ...
             ['Completed / Pixoo monitor controls', '[[P32]] [[P33]]', 'Pixoo dashboard #32 is delivered through PR #63. Issue #33 is closed after PR #64 and all five PR plus all five merged-main jobs passed. The Monitor panel provides owner-backed labels/notices, selected project/session views and exact RGB preview. Explicit Monitor activation pauses media and uses the existing generation-guarded writer; Media keeps collecting without monitor pictures. Restart and screen-on remain passive. A protected versioned extension preserves shared controller v1 and existing MCP media tools. Installed-client and physical acceptance remain [[P34]]. Its source packaging is now in progress, reusing the delivered Hub setup SDK with a disposable Pixoo rehearsal and an operator acceptance sequence. Desktop and Windows/WSL route qualification, required Claude coverage, installation and physical observations remain pending; Hub #6 gates the shared-frontend trial.'],
             ...
             '[[H6]] is in review in PR #127. Its source candidate uses the current approved Nanoleaf Prism/Neon visual language for central BUNNY activity, component and connection views. The user approved the source UI candidate. Final independent review and hosted checks remain required before source delivery. Current and future user-facing components join consistent navigation and reusable status, settings and supported-control views. A third synthetic component verifies differing capabilities; new production integrations retain their own API, UI and acceptance work.',
