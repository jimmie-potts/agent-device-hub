...
GUIDES = [
    ...
    dict(id='shared-codex', short='Shared Codex integration', title='Deliver shared Codex monitoring and integration controls', phase='Main development path',
         ...
         headers=['Stage', 'Owning issues', 'Result'], rows=[
             ...
             ['02 / Shared state and contracts', '[[H3]] [[P29]]', 'Pixoo #29 delivered adoption of the released lifecycle schema, fixtures and consumer requirements in PR #53. Hub PR #105 merged the shared agent-state source; Hub #3 remains open in this snapshot. The package supplies deterministic reduction, host storage persistence, retained notices, a bounded diagnostic journal, independent consumers and source emitters. Pixoo #31 still owns production hosting and storage adoption.'],
    ...
    dict(id='development-workflow', short='Development workflow', title='Reduce development overhead and consolidate shared tooling', phase='Maintenance',
         ...
         headers=['Group', 'Issues', 'Order'], rows=[
             ...
             ['Five Ubuntu checks', '[[H107]]', 'The owner selected Ubuntu-only CI. The candidate combines controller, lifecycle and agent-state validation into Python 3.12 and 3.14 jobs, plus one MCP job, one workflow/Linux-hook job and the work-guide browser job. Native Windows checks are removed. All existing Linux suites, package consumers and performance checks remain, with three core builds per run. Independent review, five current-head jobs and five merged-main jobs remain delivery gates; this snapshot does not claim the change is merged or measure its cost savings.'],
...
<section class="recommendation" aria-labelledby="next-title"><div class="eyebrow">Suggested next source work</div><h2 id="next-title">Complete shared-state delivery, then integrate its first host.</h2><p>''' + render('The source contract in <strong>[[H2]]</strong> is delivered. PR #77 merged the early Linux baseline and budgets for <strong>[[H30]]</strong>. <strong>[[H3]]</strong> source merged in PR #105. Its issue remains open in this snapshot; complete its remaining delivery acceptance before closure. <strong>[[P31]]</strong> owns the next host integration. Final integrated qualification in Hub #30 remains open. <strong>[[P29]]</strong> has delivered its released-contract adoption. <strong>[[P37]]</strong> remains an independent option. <strong>[[N41]]</strong> is the clearest separate feature starting point. Keep later device qualification with its owning issues. [[H63]] documentation and [[H64]] qualification are ready for future selection; this refresh starts neither.') + '''</p><p>Some blocked labels and native dependency links lag the issue bodies. Refresh readiness when selecting each issue, especially where source prerequisites have closed.</p></section>
