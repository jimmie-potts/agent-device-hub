...
parallel_section += '<p class="parallel-boundary">Keep dependent stages in order: Nanoleaf state → worker → effects; each new-device qualification → controller → status → acceptance; input qualification → local shortcuts. General-control slices wait for Hub #31 to close; music and desk presets follow their own gates.</p></section>'
