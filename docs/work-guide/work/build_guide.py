...
GUIDES = [
    ...
    dict(id='shared-codex', short='Shared Codex integration', title='Deliver shared Codex monitoring and integration controls', phase='Main development path',
         ...
         headers=['Stage', 'Owning issues', 'Result'], rows=[
             ...
             ['02 / Shared state and contracts', '[[H3]] [[P29]]', 'Pixoo #29 delivered adoption of the released lifecycle schema, fixtures and consumer requirements in PR #53. Hub #3 owns the shared runtime state interpretation using the merged early budget artifact.'],
    ...
    dict(id='development-workflow', short='Development workflow', title='Reduce development overhead and consolidate shared tooling', phase='Maintenance',
         ...
         headers=['Group', 'Issues', 'Order'], rows=[
             ...
             ['Guide-only CI', 'Completed [[H100]]', 'Hub #100 is closed and PR #101 merged the guide-only path filters. Both automatic Hub workflows exclude changes entirely under docs/work-guide/, including generators and tests. Local guide checks, independent review and applicable UI approval remain required. Mixed changes retain normal CI; verify filter applicability for each delivery.'],
             ...
             ['BUNNY system design documents', '[[H103]]', 'Prepare the linked HTML overview, component designs and combined reading view from the composable template approach. Add offline Scalar references for 37 explicit Pixoo/Nanoleaf HTTP operations and SchemaSpy reports for 28 source-defined tables. Keep delivered source, planned behavior and qualification decisions explicit. Browser request controls remain disabled pending integration with each owning service. Documentation review and current-candidate UI approval remain pending until verified; this work does not implement or deploy the described system.'],
...
<section class="recommendation" aria-labelledby="next-title"><div class="eyebrow">Suggested next source work</div><h2 id="next-title">Use the merged baseline for shared runtime design.</h2><p>''' + render('The source contract in <strong>[[H2]]</strong> is delivered. PR #77 merged the early Linux baseline and budgets for <strong>[[H30]]</strong>. Use them when selecting <strong>[[H3]]</strong>. Final integrated qualification in Hub #30 remains open. <strong>[[P29]]</strong> has delivered its released-contract adoption. <strong>[[P37]]</strong> remains an independent option. <strong>[[N41]]</strong> is the clearest separate feature starting point. Keep later device qualification with its owning issues. [[H63]] documentation and [[H64]] qualification are ready for future selection; this refresh starts neither.') + '''</p><p>Some blocked labels and native dependency links lag the issue bodies. Refresh readiness when selecting each issue, especially where source prerequisites have closed.</p></section>
