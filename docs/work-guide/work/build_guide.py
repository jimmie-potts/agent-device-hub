...
GUIDES = [
    ...
    dict(id='shared-codex', short='Shared Codex integration', title='Deliver shared Codex monitoring and integration controls', phase='Main development path',
         ...
         headers=['Stage', 'Owning issues', 'Result'], rows=[
             ...
             ['Current-status follow-up', '[[H137]]', 'Fix successive unordered Codex turns using the accepted best-effort receipt-order policy. Previously unseen delayed starts remain a known limitation. Pixoo [[P34]] waits for the delivered fix and installed update; qualification has not passed.'],
             ...
             ['Separate / Current status, task recovery and history', '[[H137]] [[H138]] [[H139]]', 'Hub #137 owns the accepted best-effort current-status revision for successive unordered Codex turns. Hub #138 follows it with an explicit per-task reset. Hub #139 is deferred refinement for reliable history; neither reset nor reliable history blocks the current-status fix or Pixoo #34. These source changes remain separate from Hub #30 timing and recovery qualification of the pinned consumers.'],
             ['06 / Everyday standalone qualification', '[[H30]] [[H9]] [[P34]]', 'Qualify the standalone Linux/WSL hub with real Pixoo/Nanoleaf consumers, synthetic input and fake device transports. Measure hook return and consumer receipt for one task and a realistic concurrent profile, with a short 50-task overload check. Cover dashboard connection/reconnect, an ordinary integration control, one unavailable consumer and host restart. Reuse existing migration and presentation regressions; omit embedded-host and production rollback qualification. The candidate command has a passing 400-warm-event source receipt: hook p95 58/78 ms for one/ten tasks, both consumer receipt p95 values below one second, and peak Hub RSS 151 MiB. All 487 hooks met the hard deadline; the 50-task burst, dashboard/reconnect, real Pixoo control, consumer outage and restart scenarios passed. Eight development attempts and all four full runs are retained. The final run adds confined preparation and positive dashboard reconnect evidence required by review. Source review, CI and merge remain pending. Practical targets were fixed before runs; retain the 3,000 ms hard hook deadline, 256 MiB service RSS ceiling, bounded resources, isolation and cleanup. Historical percentile ceilings remain diagnostic for #30. Installed-client and physical acceptance stay in their owning issues; they are not #30 closure gates.'],
         ], notes=[
             ...
             '[[H30]] deliberately spans two stages. Its delivered <strong>early budget artifact</strong> remains historical evidence; its <strong>remaining qualification</strong> now covers everyday standalone operation. [[P61]] is not a completion dependency. Standalone targets were documented before runs. The September 22 source candidate qualifies with real consumer code and fake transport; issue closure still awaits reviewed delivery and merged-source checks. Historical failures remain unchanged. Waiting for the whole issue to close before starting the runtime would create a planning deadlock.',
    ...
    dict(id='desktop-controls', short='Desktop controls', title='Add Wispr, Codex mouse actions and later desk presets', phase='Independent shortcuts, then shared presets',
         intro='[[H63]] is closed after documentation merged in PR #140. [[H64]] source qualification is in progress. Owner packaging identifies keyboard 85HA and N30 mouse 85CA. The owner reports basic operation and four distinct N30 browser outputs; receiver identity, controlled holds and app acceptance remain pending. The other seven issues are blocked by their prerequisites. <strong>Wispr and basic Codex mouse controls can ship without the hub, shared monitoring, general controls or Music.</strong> Wispr retains its own service requirements.',
         headers=['Work', 'Owning issue', 'Required prerequisites and result'], rows=[
             ['Documentation', 'Completed [[H63]]', 'No source prerequisite. The <a href="https://github.com/jimmie-potts/agent-device-hub/blob/main/docs/desktop-controls.md" target="_blank" rel="noopener noreferrer">desktop-control document</a> records accepted defaults, control profiles, desk presets and evidence boundaries. PC-lighting PR #59 is merged and remains separate.'],
             ['Hardware and Wispr qualification', '[[H64]]', 'No source prerequisites. The qualification report identifies the N Edition keyboard 85HA and N30 mouse 85CA. The N30 has an existing navigation D-pad; R8 custom-mapping claims do not apply. The authorized browser check observed PageUp, PageDown and mouse buttons 3/4. Left/Right appeared in a separate Windows browser after producing no events in Codex preview. Receiver identity, controlled holds, Big A hold/release, focus, coexistence and restoration remain unresolved. [[H65]] remains blocked.'],
