...
GUIDES = [
    ...
    dict(id='pc-lighting', short='PC + desk lighting', title='Add PC and desk lighting, starting with Corsair', phase='Future qualification and delivery',
         ...
         headers=['Track', 'Owning issues', 'Required work and prerequisites'], rows=[
             ...
             '<a href="https://github.com/jimmie-potts/agent-device-hub/pull/59" target="_blank" rel="noopener noreferrer">PC-lighting documentation PR #59</a> is merged and remains separate from desktop controls. All eleven follow-up issues are deferred backlog work requiring refinement; this guide performs no qualification, installation or device operations.',
         ]),
    dict(id='desktop-controls', short='Desktop controls', title='Add Wispr, Codex mouse actions and later desk presets', phase='Independent shortcuts, then shared presets',
         intro='[[H63]] is closed after documentation PR #140 merged. [[H64]] is in review and still blocked; its open PR does not establish acceptance. The other seven issues retain their own prerequisites. <strong>Wispr and basic Codex mouse controls can ship without the hub, shared monitoring, general controls or Music.</strong> Wispr retains its own service requirements.',
         headers=['Work', 'Owning issue', 'Required prerequisites and result'], rows=[
             ...
             ['Hardware and Wispr qualification', '[[H64]]', 'Qualification is under review in Hub PR #143. Resolve missing held-input, app-focus and shortcut evidence before implementation; read the issue for the current model findings and separately authorized checks.'],
             ['Independent / Local controls', '[[H65]]', 'Requires [[H63]] and [[H64]]. Version 1 defaults: Big B holds Ctrl+Win for Wispr and releases to insert. Big A sends Enter only on a separate deliberate press. Both bindings are user-editable; dictation release never automatically sends Enter. Codex mouse actions: next task needing attention, command menu, previous task and next task. Preserve ordinary mouse behavior outside Codex.'],
             ...
             ['Shared / Selected preset binding and feedback', '[[H68]]', 'Requires [[H65]] and [[H67]]. An explicitly selected qualified binding cycles Work → Free → Quiet → Work through the hub, with visible requested and partial/failed/uncertain results. Manual dispatch does not require automation engine [[H45]].'],
    ...
    dict(id='controls-music', short='Controls + music', title='Expand into general device controls and Apple Music', phase='After Codex acceptance',
         ...
         headers=['Stage', 'Issues', 'Work'], rows=[
             ...
             ['Pixoo controls', '[[H151]] → [[H154]]', 'Hub PR #160 carries the source candidate for review: screen power, brightness, saved-playlist selection and the six playback actions in the existing Pixoo view, one guarded controller v1 command each through the existing hub route, with Monitor gating and an explicit Media switch through the Pixoo integration mode operation because the real controller declares controller v1 modes unsupported. Independent reviews, hosted checks and explicit human approval of the actual UI candidate remain open; nothing is installed or operated. Physical acceptance on the display is [[H154]].'],
             ['Completed / Nanoleaf native controls', '[[N64]]', 'Nanoleaf #64 closed as completed on September 22, 2026 after PR #66 merged as 8062849 with four rounds of independent Standards and Specification review and all nine merged-main checks passing. The protected controller declares power, brightness (0 to 100) and discovered saved scenes (opaque IDs, at most 256) as supported controller v1 capabilities and executes them as one journaled write each through the existing single worker. Power and brightness are overrides that govern the worker’s writes in the current mode until the next explicit mode command from any owner, including the same mode; while power is off the worker keeps tracking tasks and writes nothing. Scene activation is accepted only in Free, fails typed as unsupported-capability in Work or Quiet before any write, and adds no polling; scene names travel only in the nanoleaf.integration/1.0 snapshot, whose new scenes list the hub validator accepts. Source tests with fake devices only; the installed wall is [[H155]].'],
             ['Nanoleaf controls', '[[H153]] → [[H155]]', 'Hub PR #162 carries the source candidate for review: power, brightness and saved-scene activation in the existing Nanoleaf view on the capabilities Nanoleaf #64 declares, one guarded controller v1 command each through the existing hub route. Brightness is shown as an override until the next explicit mode command; scenes are disabled in Work and Quiet or while a mode change is pending, with an explicit Free switch through the controller v1 mode command, and are labelled by user-chosen names from the integration snapshot or by ID. The candidate is stacked on the Nanoleaf #64 hub companion PR #161 and the Pixoo PR #160 and merges only after them. Independent reviews, hosted checks and explicit human approval of the actual UI candidate remain open; nothing is installed or operated. Physical acceptance on the installed wall is [[H155]].'],
...
parallel_section += '<p class="parallel-boundary">Keep dependent stages in order: Nanoleaf state → worker → effects; each new-device qualification → controller → status → acceptance; input qualification → local shortcuts. Pixoo controls, Nanoleaf capabilities and Pixoo playlist names are selectable now; the Nanoleaf view and both hardware acceptances follow their prerequisites; music and desk presets follow their own gates.</p></section>'
...
<article><h3>Next decision</h3><p>Hub #32 and the general-control definition Hub #31 are closed. Choose among the Pixoo slice #151, Nanoleaf #64 and Pixoo #67, which have no open prerequisites.</p><a href="#controls-music">Review general controls</a></article>
<article><h3>Independent work</h3><p>Nanoleaf device foundations, Pixoo embedded performance and separate device qualification can be selected alongside the general-control slices.</p><a href="#parallel-work">Compare parallel units</a></article></div>
