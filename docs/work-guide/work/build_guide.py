...
GUIDES = [
    ...
    dict(id='shared-codex', short='Shared Codex integration', title='Deliver shared Codex monitoring and integration controls', phase='Main development path',
         ...
         headers=['Stage', 'Owning issues', 'Result'], rows=[
             ['01 / Qualification and budgets', '[[H2]] [[H30]]', 'Hub #2 delivered lifecycle contract v1, portable validators and shared fixtures in PR #74, with passing merged-main CI and a checksum-pinned release. Hub #30 has a Linux source candidate in PR #77: 9,000 hook calls across repeated 1/10/50-session profiles, verified worker cleanup, and numeric budget definitions. Independent review, source merge and main CI must accept the early budget before core implementation. Final feed/consumer qualification remains pending. Native Windows comparison and executable forwarding are no longer required; CI platform cleanup is separate. Documented provider support does not prove installed-client compatibility.'],
    ...
    dict(id='development-workflow', short='Development workflow', title='Reduce development overhead and consolidate shared tooling', phase='Maintenance',
         ...
         headers=['Group', 'Issues', 'Order'], rows=[
             ...
             ['Guide-only CI', '[[H100]]', 'Planned: skip both Hub CI workflows when every changed path is under docs/work-guide/, including generators and tests. Retain local guide checks, review evidence and applicable UI approval. Mixed changes keep full CI; required-check compatibility and hosted skipping remain delivery gates.'],
