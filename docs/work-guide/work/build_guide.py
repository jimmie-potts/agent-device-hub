...
GUIDES = [
    ...
    dict(id='shared-codex', short='Shared Codex integration', title='Deliver shared Codex monitoring and integration controls', phase='Main development path',
         ...
         headers=['Stage', 'Owning issues', 'Result'], rows=[
             ...
             ['Completed / Stale approval recovery', '[[H185]] [[N72]]', 'Hub PR #187 merged as eeaa764 and all six merged-main jobs passed. Its protected revision-guarded command retires one uncertain unknown-ID approval for an exact session and turn, retaining a distinct durable recovery receipt; it does not grant or deny Codex permission. Nanoleaf PR #73 merged as 680acb5 and all five merged-main jobs passed. A later owner revision clears the frozen red projection while `statusEvidence` remains uncertain, without replaying a comet. Fake Line frames were red before and during uncertainty and blue after recovery. These are source checks only: the installed bridge and physical Lines were not changed, and the public guide edition has not been republished.'],
             ['Investigate retained unread tasks', '[[N74]]', 'The installed wall map counted 72 visible tasks in one read-only snapshot: 65 unread, five blocked and two working. [[N74]] will trace whether unread notices remain actionable before changing retention or presentation. It does not clear notices by age or claim this approval recovery fixed the count.'],
    ...
    dict(id='tidbyt-lifx', short='Tidbyt + LIFX', title='Add automatic status to Tidbyt and LIFX', phase='Additional devices',
         ...
         headers=['Track', 'Issue sequence', 'Work and dependencies'], rows=[
             ...
             'LIFX connection qualification can start independently. The shared-state, host and lifecycle prerequisites [[H3]], [[P31]] and [[H8]] are delivered; Tidbyt automatic status [[H19]] can start on the delivered controller; LIFX automatic status waits on its controller [[H18]]. LIFX model identification remains part of qualification.',
    ...
    dict(id='development-workflow', short='Development workflow', title='Reduce development overhead and consolidate shared tooling', phase='Maintenance',
         ...
         headers=['Group', 'Issues', 'Order'], rows=[
             ...
             ['Atlas and diagram readability', 'Completed [[H172]] [[H173]] → [[H186]]', 'Hub #172 delivered the atlas system map and one agent-observation walkthrough through PR #177. Hub #173 closed as completed after PR #184 merged as 0c4cdef with the remaining seven guide diagrams made readable and consistent. [[H186]] now owns a reviewed source export and navigation for a future public atlas; public copying and Pages verification remain in the public repository issue. The reviewed source export and atlas navigation are in this revision; the public repository issue owns copying and live Pages verification.'],
...
<title>Agent device work guides · @@MONTH_TITLE@@</title><style>''' + CSS + '''
.atlas-link{display:block;margin:14px 8px 6px;padding:11px 10px;border:1px solid var(--edge);color:var(--accent);font-size:12px;line-height:1.4;text-decoration:none}.atlas-link:hover,.atlas-link:focus-visible{color:var(--text);border-color:var(--accent);background:#22d3ee0b}</style></head>
