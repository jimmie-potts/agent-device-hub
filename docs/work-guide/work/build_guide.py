...
GUIDES = [
    ...
    dict(id='controls-music', short='Controls + music', title='Expand into general device controls and Apple Music', phase='After Codex acceptance',
         ...
         headers=['Stage', 'Issues', 'Work'], rows=[
             ['General-control definition', '[[H31]]', 'Accepted on September 22, 2026 and recorded in hub ADR 0005. Controls stay in each component’s existing BUNNY view from [[H6]] and submit one guarded controller v1 command each. Power and brightness are mode-independent; playlist, playback and scene controls are disabled while a device presents agent status, with an explicit switch to Media or Free; nothing restores automatically; availability is declared capability times existing control scope. The issue closes once its documentation PR merges.'],
...
<article><h3>Next decision</h3><p>Hub #32 is closed. General controls are defined under Hub #31; the Pixoo slice #151, Nanoleaf #64 and Pixoo #67 become selectable once #31 closes.</p><a href="#controls-music">Review general controls</a></article>
