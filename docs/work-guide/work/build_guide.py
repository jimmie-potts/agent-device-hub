...
GUIDES = [
    ...
    dict(id='shared-codex', short='Shared Codex integration', title='Deliver shared Codex monitoring and integration controls', phase='Main development path',
         ...
         headers=['Stage', 'Owning issues', 'Result'], rows=[
             ...
             ['Parallel / Pixoo display qualification', '[[P30]]', 'Pixoo PR #59 merged the source tools for #30: an offline-default bounded frame runner, exact RGB payload previews and shared local writer exclusion. The Windows cancellation-test deadlock was corrected using controlled timers and unconditional import-gate cleanup; all five PR checks passed. Source delivery does not establish physical acceptance. Text/item execution remains unavailable because exact device-font preview is unqualified. Physical latency, interruptions, recovery and the measured method/cadence decision remain pending before #33 device integration. Pure rendering and simulator work can proceed earlier.'],
             ['Parallel / Embedded-host performance', '[[P61]] after [[P31]]', 'Audit measurement boundaries, diagnose and optimize bottlenecks, then qualify frozen limits. Preserve failed receipts; this hardening does not block feature delivery.'],
             ['05 / Shared hosting and clients', '[[H5]] [[H8]] [[H6]] [[H13]]', 'Deliver the standalone host, reversible hook migration, common frontend, and shared MCP discovery/status tools.'],
    ...
    dict(id='assistant-access', short='Assistant + access', title='Add automation, conversation, and later access methods', phase='Future refinement',
         ...
         headers=['Track', 'Issues', 'Suggested progression'], rows=[
             ...
             ['Personal services', '[[H49]]', 'Select and qualify a first connector use case.'],
...
METADATA = dict(refreshedAt=SNAPSHOT['refreshedAt'], staticSnapshot=True, openIssues=TOTAL,
                ...
                primaryCoverage=coverage, completedBaselines=['P12', 'P26', 'P29', 'P37', 'H2', 'H4', 'H7'],
...
<section class="recommendation" aria-labelledby="next-title"><div class="eyebrow">Suggested next source work</div><h2 id="next-title">Integrate the released state package into its first host.</h2><p>''' + render('The source contract in <strong>[[H2]]</strong> is delivered. PR #77 merged the early Linux baseline and budgets for <strong>[[H30]]</strong>. <strong>[[H3]]</strong> is closed. PR #105 merged its source, agent-state 1.0.0 is released, and all five applicable merged-main checks passed after PR #108 repaired CI setup. The state package is ready for separately scoped host adoption. <strong>[[P31]]</strong> is now in progress for that host integration. Its candidate adds private durable ownership, authenticated admission, selected-owner reads and explicit cutover. The owner deferred embedded-host performance measurement and numeric acceptance to <strong>[[P61]]</strong>; it follows #31 and does not block feature delivery. Failed measurements and frozen limits remain recorded. Final integrated qualification in Hub #30 remains open. <strong>[[P29]]</strong> has delivered its released-contract adoption. <strong>[[P37]]</strong> is closed after PR #58 delivered the protected shared controller boundary; host and frontend adoption remain with their owning issues. <strong>[[N41]]</strong> is the clearest separate feature starting point. Keep later device qualification with its owning issues. [[H63]] documentation and [[H64]] qualification are ready for future selection; this refresh starts neither.') + '''</p><p>Some blocked labels and native dependency links lag the issue bodies. Refresh readiness when selecting each issue, especially where source prerequisites have closed.</p></section>
