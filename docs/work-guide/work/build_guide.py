...
GUIDES = [
    ...
    dict(id='shared-codex', short='Shared Codex integration', title='Deliver shared Codex monitoring and integration controls', phase='Main development path',
         ...
         headers=['Stage', 'Owning issues', 'Result'], rows=[
             ...
             ['Completed / Standalone source qualification', '[[H30]]; later [[H9]] [[P34]]', 'The standalone Linux/WSL source qualification uses real Pixoo/Nanoleaf consumers, synthetic input and fake device transports. It measures hook return and consumer receipt for one task and a realistic concurrent profile, with a short 50-task overload check. It covers dashboard connection/reconnect, an ordinary integration control, one unavailable consumer and host restart. Reuse existing migration and presentation regressions; omit embedded-host and production rollback qualification. The delivered command has a passing 400-warm-event source receipt: hook p95 58/78 ms for one/ten tasks, both consumer receipt p95 values below one second, and peak Hub RSS 151 MiB. All 487 hooks met the hard deadline; the 50-task burst, dashboard/reconnect, real Pixoo control, consumer outage and restart scenarios passed. Eight development attempts and all four full runs are retained. The final run adds confined preparation and positive dashboard reconnect evidence required by review. PR #135 merged as 8411413 after both independent reviews and six PR jobs passed. All six merged-main jobs passed before #30 closed. Practical targets were fixed before runs; retain the 3,000 ms hard hook deadline, 256 MiB service RSS ceiling, bounded resources, isolation and cleanup. Historical percentile ceilings remain diagnostic for #30. Installed-client and physical acceptance stay in their owning issues; they are not #30 closure gates.'],
         ], notes=[
             ...
             '[[H30]] completed two source stages. Its <strong>early budget artifact</strong> remains historical evidence; its <strong>standalone qualification</strong> covers everyday standalone operation and is delivered through PR #135. [[P61]] was not a completion dependency. Standalone targets were documented before runs. The September 22 receipt qualifies real consumer code with fake transport. Historical failures remain unchanged; installed-client and physical evidence retain their own gates.',
