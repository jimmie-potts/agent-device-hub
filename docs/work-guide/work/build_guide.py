...
<section class="recommendation" aria-labelledby="next-title"><div class="eyebrow">Suggested next source work</div><h2 id="next-title">Review the shared state package, then integrate its first host.</h2><p>''' + render('The source contract in <strong>[[H2]]</strong> is delivered. PR #77 merged the early Linux baseline and budgets for <strong>[[H30]]</strong>. <strong>[[H3]]</strong> is implementing the shared state package against those frozen inputs. Review and merge gates remain pending; <strong>[[P31]]</strong> owns the next host integration. Final integrated qualification in Hub #30 remains open. <strong>[[P29]]</strong> has delivered its released-contract adoption. <strong>[[P37]]</strong> remains an independent option. <strong>[[N41]]</strong> is the clearest separate feature starting point. Keep later device qualification with its owning issues. [[H63]] documentation and [[H64]] qualification are ready for future selection; this refresh starts neither.') + '''</p><p>Some blocked labels and native dependency links lag the issue bodies. Refresh readiness when selecting each issue, especially where source prerequisites have closed.</p></section>
