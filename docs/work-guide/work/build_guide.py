...
GUIDES = [
    ...
    dict(id='shared-codex', short='Shared Codex integration', title='Deliver shared Codex monitoring and integration controls', phase='Main development path',
         ...
         headers=['Stage', 'Owning issues', 'Result'], rows=[
             ...
             ['02 / Shared state and contracts', '[[H3]] [[P29]]', 'Pixoo #29 delivered adoption of the released lifecycle schema, fixtures and consumer requirements in PR #53. Hub PR #105 merged the shared state source on September 20 UTC. The <a href="https://github.com/jimmie-potts/agent-device-hub/releases/tag/agent-state-v1.0.0" target="_blank" rel="noopener noreferrer">agent-state 1.0.0 release</a> includes the reproducible archive, checksum and source receipt. It supplies deterministic reduction, host storage persistence, retained notices, a bounded diagnostic journal, independent consumers and source emitters. Twelve merged-revision CI jobs now pass. Ubuntu Linux-hook setup failed while downloading bubblewrap; Hub #107 / PR #108 owns the repair. Hub #3 remains open until the applicable merged-main checks establish acceptance. Pixoo #31 owns production hosting and storage adoption; Hub #8 owns installed-provider qualification.'],
             ...
             ['Parallel / Pixoo display qualification', '[[P30]]', 'Measure the update method and cadence needed for final Pixoo #33 device integration. Pure rendering and simulator work can proceed earlier.'],
    ...
    dict(id='development-workflow', short='Development workflow', title='Reduce development overhead and consolidate shared tooling', phase='Maintenance',
         ...
         headers=['Group', 'Issues', 'Order'], rows=[
             ['System design documents', '[[H103]]', 'The separately owned PR #104 develops the BUNNY system-design atlas and API/database references. Its current review, UI and CI gates remain with that issue.'],
             ['Hub CI consolidation', '[[H107]]', 'Separately owned work will consolidate normal CI from thirteen jobs to five Ubuntu jobs while retaining the existing suites and Python 3.12/3.14 coverage. This changes future CI platform support, not Windows client or device ownership. Its own candidate and merged-main checks remain required; this guide update does not implement that change.'],
...
<section class="recommendation" aria-labelledby="next-title"><div class="eyebrow">Suggested next source work</div><h2 id="next-title">Complete shared-state CI acceptance, then integrate its first host.</h2><p>''' + render('The source contract in <strong>[[H2]]</strong> is delivered. PR #77 merged the early Linux baseline and budgets for <strong>[[H30]]</strong>. PR #105 merged the <strong>[[H3]]</strong> state package, and agent-state 1.0.0 is released. Its source checklist is complete and twelve merged-revision CI jobs pass. Ubuntu Linux-hook setup still needs the package-index repair owned by Hub #107 / PR #108. Complete CI acceptance before closing #3; <strong>[[P31]]</strong> owns the next host integration. Final integrated qualification in Hub #30 remains open. <strong>[[P29]]</strong> has delivered its released-contract adoption. <strong>[[P37]]</strong> remains an independent option. <strong>[[N41]]</strong> is the clearest separate feature starting point. Keep later device qualification with its owning issues. [[H63]] documentation and [[H64]] qualification are ready for future selection; this refresh starts neither.') + '''</p><p>Some blocked labels and native dependency links lag the issue bodies. Refresh readiness when selecting each issue, especially where source prerequisites have closed.</p></section>
