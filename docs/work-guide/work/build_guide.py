...
GUIDES = [
    ...
    dict(id='shared-codex', short='Shared Codex integration', title='Deliver shared Codex monitoring and integration controls', phase='Main development path',
         ...
         headers=['Stage', 'Owning issues', 'Result'], rows=[
             ...
             ['03 / First host and controller API', '[[P31]]; completed [[P37]]', 'Pixoo PR #58 delivered the optional protected controller v1 API through its existing ledger, player and serialized writer. Issue #37 is closed after all five PR and all five merged-main jobs passed, with 515 application tests and 52 browser tests passing locally. Configured identity, scoped authentication, replay, bounded feeds and cross-client media evidence are implemented. Pixoo #31 is in progress for shared state hosting with private durable storage and a selected embedded/remote source. Local Linux measurements exceed its fixed percentile budgets, so that source candidate remains blocked from acceptance and merge. Installation, real hub-client operation and physical acceptance remain separate.'],
             ...
             ['Parallel / Pixoo display qualification', '[[P30]]', 'Pixoo #30 has a reviewed source candidate in PR #59: an offline-default bounded frame runner, exact RGB payload previews and shared local writer exclusion. Source merge is blocked by a Windows media-admission cancellation-test timeout after one failed retry; the other four jobs passed. Text/item execution remains unavailable because exact device-font preview is unqualified. Physical latency, interruptions, recovery and the measured method/cadence decision remain pending before #33 device integration. Pure rendering and simulator work can proceed earlier.'],
