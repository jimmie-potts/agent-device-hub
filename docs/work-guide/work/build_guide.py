...
GUIDES = [
    ...
    dict(id='development-workflow', short='Development workflow', title='Reduce development overhead and consolidate shared tooling', phase='Maintenance',
         ...
         headers=['Group', 'Issues', 'Order'], rows=[
             ...
             ['Hub CI efficiency', '[[H91]]', 'Core scheduling and build-once test commands merged through Hub PRs #92 and #93. Every test and platform remains. The matching guide workflow merged in PR #82 and passed all configured main jobs. Hub #91 retains its own completion and measurement record. Linux time improved in one sample; Windows variability prevents a per-run billing-savings claim.'],
