...
GUIDES = [
    ...
    dict(id='shared-codex', short='Shared Codex integration', title='Deliver shared Codex monitoring and integration controls', phase='Main development path',
         intro='[[H32]] is the completed guide for this milestone. It closed on September 22, 2026 after every child story in its dependency table was closed, including Pixoo #34 and Nanoleaf #30 installed acceptance. This is the main cross-project development sequence; its remaining open items are the BUNNY launcher and style work plus hardening and recovery follow-ups.',
         headers=['Stage', 'Owning issues', 'Result'], rows=[
             ...
             ['BUNNY owner launcher', '[[H179]]', 'Open the local dashboard with one click through a short-lived private handoff from the running Hub, without typing a browser token, while keeping scoped Hub authentication. Source merged in PR #180 as 1338c03 on September 23, 2026; the issue stays in review until its closeout, and installation is separate.'],
    ...
    dict(id='nanoleaf-devices', short='Lines + Light Panels', title='Add Nanoleaf Light Panels alongside Lines', phase='Independent feature',
         ...
         headers=['Order', 'Issue', 'Result'], rows=[
             ...
             ['Closed / Runtime worker source', '[[N42]]', 'Nanoleaf #42 closed as not planned on September 23, 2026 after its device-worker scope was folded into [[N43]], which delivered it. Installation and physical acceptance retain their own issues.'],
    ...
    dict(id='tidbyt-lifx', short='Tidbyt + LIFX', title='Add automatic status to Tidbyt and LIFX', phase='Additional devices',
         ...
         headers=['Track', 'Issue sequence', 'Work and dependencies'], rows=[
             ['Tidbyt cloud', '[[H16]] → [[H19]] → [[H21]]', 'Hub #16 closed as completed on September 23, 2026 after PR #174 merged as c1f71be with independent Standards and Specification approval and all six PR and merged-main checks passing. It absorbed the connection qualification from [[H15]]: one owner-observed background push joined the rotation without interrupting the current app, and its installation was deleted. The delivered in-process controller has a pure 64×32 WebP renderer, a background-push cloud connection with private credentials and one serialized queue; it is tested with fakes only. Automatic status [[H19]] settles layout, push cadence and rotation with the owner; installed acceptance [[H21]] follows with its own authorization.'],
             ...
             'LIFX connection qualification can start independently. The shared-state, host and lifecycle prerequisites [[H3]], [[P31]] and [[H8]] are delivered; automatic status waits on each device controller. LIFX model identification remains part of qualification.',
    ...
    dict(id='development-workflow', short='Development workflow', title='Reduce development overhead and consolidate shared tooling', phase='Maintenance',
         ...
         headers=['Group', 'Issues', 'Order'], rows=[
             ...
             ['Atlas and diagram readability', 'Completed [[H172]] → [[H173]]', 'Hub #172 closed as completed on September 23, 2026 after PR #177 merged as d8527cd: the BUNNY atlas opens with a system map and one agent-observation walkthrough built from the shared diagram definitions. Next, [[H173]] applies that pattern to the remaining guide diagrams.'],
...
architecture_section = f'''<details class="reference" id="architecture" data-diagrams="{len(AD.DIAGRAMS)}" open>
      ...
      <div class="guide-body"><p class="intro">Three system diagrams and six sequence diagrams show the implemented local paths, the Nanoleaf Linux runtime and how the shared system composes them. The system map (A2) and the agent observation walkthrough (S4) were revised on 23 September 2026 against hub main 5db67a09; the BUNNY design atlas embeds these same definitions. Nanoleaf installed acceptance is recorded in {render('[[N55]]')}; on 23 September 2026 its Linux runtime viewer and the owner-migration and desktop-input status labels were updated to current issue state. The desktop-input and preset diagrams follow the pinned keyboard-owned boundary: B.U.N.N.Y. handles qualified mouse controls, while keyboard A/B and attached Super Buttons stay outside its configuration, dispatcher and preset bindings. Diagram source pins are unchanged. Baseline sources were reviewed at <time datetime="{html.escape(AD.SOURCES['reviewedAt'])}">@@REVIEW_TIMESTAMP@@</time> against the pinned revisions listed on each diagram. Repository history was read separately at @@HISTORY_TIMESTAMP@@; its main revisions may be newer than the architecture review. Diagram links repeat issues that already belong to a work guide; they add nothing to the issue totals. Each figure has zoom and fit controls, a scrollable stage, a text explanation and a link to the full interactive Archify viewer shipped beside this file in the architecture folder. Those viewers are Archify's own HTML: they reference one Google Fonts stylesheet and fall back to system fonts when offline; this guide itself loads nothing remote.</p>
