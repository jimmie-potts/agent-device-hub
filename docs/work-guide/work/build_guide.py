...
architecture_section = f'''<details class="reference" id="architecture" data-diagrams="{len(AD.DIAGRAMS)}" open>
      ...
      <div class="guide-body"><p class="intro">Three system diagrams and six sequence diagrams show what runs where, who owns state and who writes to each device. Start with the system map (A2) and the agent observation walkthrough (S4); the BUNNY design atlas embeds these same two definitions, which keep their hub main 5db67a09 pins. The other seven views were revised on 23 September 2026 under {render("[[H173]]")} against Hub d8527cd3, Nanoleaf cbb94851 and Pixoo c81bc31c, following the same pattern: dashed boxes are processes, devices sit outside them, and each sequence keeps a short numbered main path with alternatives in separate lanes and a walkthrough below. On 24 September 2026 the Nanoleaf runtime view (A3) took the installed evidence from {render("[[N46]]")} and {render("[[N89]]")} without moving its pins. The system map keeps its earlier pin: since then, Tidbyt agent status is installed ({render("[[H21]]")}), the LIFX controller source is delivered ({render("[[H18]]")}) and shared playback with the Sony source is installed ({render("[[H175]]")}), none of which A2 draws yet. Current, historical and planned behavior are labelled separately: the Nanoleaf views show the installed Linux runtime ({render("[[N55]]")}) and name the retired Windows route as historical, and the two future scenarios sit behind a marked, collapsed group. Each figure lists the revisions its sources are pinned to. The baseline review time is <time datetime="{html.escape(AD.SOURCES['reviewedAt'])}">@@REVIEW_TIMESTAMP@@</time>. Repository history was read separately at @@HISTORY_TIMESTAMP@@; its main revisions may be newer than the architecture review. Diagram links repeat issues that already belong to a work guide; they add nothing to the issue totals, and their status badges start from the dated backlog snapshot and update from public GitHub when the page opens. Each figure has zoom and fit controls, a scrollable stage, a text explanation and a link to the full interactive Archify viewer shipped beside this file in the architecture folder. Those viewers are Archify's own HTML: they reference one Google Fonts stylesheet and fall back to system fonts when offline.</p>
...
document = document.replace('<script>', f'<script id="snapshot-data" type="application/json">{metadata_json}</script><script id="issue-briefs" type="application/json">{briefs_json}</script><script>')
