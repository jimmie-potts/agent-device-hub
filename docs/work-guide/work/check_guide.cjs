...
(async()=>{
  ...
  try {
    ...
    await screenshot('desktop'); await page.locator('#collapse-all').click();
    ...
    await page.locator('#search').press('Escape'); assert.deepEqual(await expansion(),beforeSearch); assert.equal(await page.locator('.diagram:not([hidden])').count(),diagramIds.length); assert(await page.locator('#timeline').isVisible());
    ...
    const uiState=()=>page.evaluate(()=>({guides:[...document.querySelectorAll('.guide,.reference')].map(e=>({open:e.open,hidden:e.hidden})),figures:[...document.querySelectorAll('.diagram')].map(e=>e.hidden),zoom:[...document.querySelectorAll('.diagram-canvas > svg')].map(e=>e.style.width),nav:[...document.querySelectorAll('nav a')].map(e=>e.hidden),search:document.querySelector('#search').value,result:document.querySelector('#result-count').textContent,empty:document.querySelector('#empty-state').hidden,clear:document.querySelector('#clear-search').hidden}));
