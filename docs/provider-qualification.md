...
[Hub #30](https://github.com/jimmie-potts/agent-device-hub/issues/30) measures Windows-native, WSL-native and selected cross-boundary producer routes using actual runtimes and this delivered contract. Record cold/warm starts, repeat counts, load and artifact revisions. Separate producer return, admission, reduction, publication, consumer admission and transport. Compare durations within one monotonic clock domain. This document supplies no measurements or numeric budgets. Freeze measured budgets before Hub #3; installed-client and integrated qualification retain their own gates.
