...
The shared owner retires the known record and its known descendants on a normalized `runtime.ended` from every supported path. It does not classify the reason and adds no turn-duration timer ([Hub #241](https://github.com/jimmie-potts/agent-device-hub/issues/241), extending [Hub #218](https://github.com/jimmie-potts/agent-device-hub/issues/218)). The table records the documented end and start evidence per path, read on 2026-09-25. Source normalization and synthetic host checks establish the mapping; neither proves delivery by an installed build. A path with missing or unqualified end evidence is a coverage gap. Its records keep the 24-hour expiry fallback, and the owner never invents an end.
