...
Status: accepted. The owner accepted the shared rules (AC2) and the component
glossary (AC8) on 2026-09-25 on the delivery pull request for
[Hub #181](https://github.com/jimmie-potts/agent-device-hub/issues/181).
The observed values below are source facts, the shared rules bind every
B.U.N.N.Y. application page, and device exceptions stay with their owning
repository.
...
([PR #251](https://github.com/jimmie-potts/agent-device-hub/pull/251)) after
the revision above; where this guide quotes the fixed-meaning token values,
those files now hold them.
...
Role names are shared with the documentation skin file
`docs/skins/neon-geometry-wars.css` (Hub #85). Application code uses the same names, held in the application's own token
...
surfaces: `--muted` `#93a6cc`, `--inset` `#060a14`, `--panel-translucent`
`rgba(8,12,22,.9)`, `--edge-strong` `rgba(34,211,238,.45)` and `--edge-faint`
`rgba(34,211,238,.09)`. The role names and meanings are the same.
...
| Form field | Dark input, pale text, visible label, white focus outline. Since Hub #277 there is no Apply button: a select sends a pointer choice at once and a keyboard step after a pause, a slider once when released, a picker when it closes, a text field when left or on Enter while its constraints hold. | Labels stay visible; show the current or unknown value honestly; the control shows the current value again once its result arrives; keep an unsent text draft and selection across a refresh. |
...
reviewed them for the AC2 decision.
...
of B.U.N.N.Y. The owner accepted the dispositions below on 2026-09-25,
recorded on the delivery pull request. Every
...
| Term | Disposition | Plain meaning | Technical responsibility | Owner | Intended use |
| --- | --- | --- | --- | --- | --- |
