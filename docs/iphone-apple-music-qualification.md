...
| Check | Observation |
| --- | --- |
| API inventory | `avContent.getMethodTypes` listed `setPlayContent` v1.2 with `uri`, `output`, `resume`, `requester`, position and repeat parameters, plus `pausePlayingContent` v1.1, `stopPlayingContent` v1.1, `scanPlayingContent` v1.0 and next/previous v1.0. `getAvailablePlaybackFunction` reported `playback`, `stop`, `pause`, `fwdScan`, `bwdScan`, `next` and `prev` as available on `extInput:airPlay` while paused. `getSupportedPlaybackFunction` returned empty function lists for all six sources and rejected any specific URI. |
| `setPlayContent` | With AirPlay paused from the phone, twelve forms were each rejected with `[3, "Illegal Argument"]`. With `output` empty: `uri` empty with `resume: true`, `resume: true` alone, `uri` empty with `resume: true` and `requester: "ui"`, `uri` `extInput:airPlay` with `resume: true`, `uri` `extInput:airPlay` alone, and `uri` `extInput:airPlay` with `requester: "ui"`. With `output` `extOutput:zone?zone=1`: `uri` empty with and without `resume: true`, `uri` `extInput:airPlay` with and without `resume: true`, `output` alone, and an empty parameter object. [Sony's Audio Control API reference](https://developer.sony.com/develop/audio-control-api/api-references/api-overview-2) describes the empty URI as the resume form for content the receiver plays itself. |
| `pausePlayingContent` | The same reference documents it as a play/pause toggle. Sent while paused from the phone, with both output forms, it returned an empty success result and the state stayed `PAUSED`. Sent four seconds after a hub-issued pause, it returned success and the state stayed `PAUSED` for eight seconds. |
| `scanPlayingContent` | `direction: "fwd"` returned success and changed nothing. |
| Other control paths | No UPnP AVTransport: a unicast SSDP search sent to the receiver got no reply and port 52323 refused the connection. No IRCC: ports 50001, 50002, 50201 and 50202 refused, and the Audio Control API port returned 404 for IRCC paths. Of the checked ports, only the Cast ports and the Audio Control API port accepted a connection. The [remote control guide](https://helpguide.sony.net/ht/a9/v1/en/contents/TP1000029196.html) documents the Play/Pause button as resuming playback, but the API has no key-injection method, and the button was not checked. |

Result: the HT-A9 cannot resume a paused AirPlay session over its network API. The `setPlayContent` forms were tried in the phone-paused state only, but their rejections were argument errors, and the toggle failed after both a phone pause and a hub pause. The Sony source keeps play unavailable.
