...
// The publisher only follows owner snapshots, so every supported path retires the same way (Hub #241).
for (const [provider, client] of [['codex', 'desktop'], ['codex', 'cli'], ['claude', 'code']])
test(`${provider}/${client} retirement releases the installation on current empty reconnect without confusing feed loss`, async t => {
  let available=true;
  const s=await setup(t,{feed:owner=>({snapshot:()=>available?owner.snapshot():undefined})});
  const ended=(name,turn)=>normalizeHook({session_id:'ended',turn_id:turn,prompt_id:turn},
    {provider,client,hostId:'host',sourceId:'source',hook:name},++ownerClock.now);
  await s.ingest(ended('UserPromptSubmit','old'));
  s.publisher.start();await s.settle();
  assert.equal(s.publisher.state().installation,'present');
  available=false;await s.advance(20*SECOND);
  await s.ingest(ended('SessionEnd','old'));
  ...
  await s.ingest(ended('UserPromptSubmit','fresh'));
  await s.advance(MINUTE);
