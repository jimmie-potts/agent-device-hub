...
export function fontText(text: string): string {
  return [...text.toUpperCase()].map(ch => Object.hasOwn(GLYPHS, ch) ? ch : '-').join('');
}

/** The glyph rows for one character already mapped by `fontText`. */
export function glyph(ch: string): readonly number[] {
