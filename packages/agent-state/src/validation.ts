...
export function validateExport(input:unknown):Validation<DurableState>{
  return validate(input,durableCheck,state=>{
    ...
    for(const session of state.sessions){
      if(session.lastEvidenceAtMs>state.lastCommitAtMs||session.notices.some(n=>n.acknowledgedBy.some(id=>!consumers.has(id))))return false;
    }
