...
function observedResult(source:unknown,ticket:unknown,device=true):ReturnType<typeof resultMessage>|undefined{
 ...
 if(result&&result.outcome!=='queued')return resultMessage(result,{device});
}
...
function useCommand(api:Api,path:string,refresh:()=>Promise<unknown>,source:unknown){
 ...
 useEffect(()=>{const result=submitted&&observedResult(source,submitted.ticket);if(!result)return;setStatus(`${submitted.label}: ${result.message}`);setTone(result.settled);if(result.locked)setLocked(true);},[source,submitted]);
 async function run(label:string,prepare:()=>Promise<Prepared<{request:unknown}>>){if(busy||locked)return;keepFocus();setBusy(true);setTone('pending');setStatus(`${label}: Sending…`);setSubmitted(undefined);
  try {
   ...
   let result:ReturnType<typeof resultMessage>;try{result=resultMessage(await api.request<ReceiptEvidence>(path,prepared.request));}catch(error){result=failureMessage(error);}
...
function ComponentView({component,device,context,api,refresh,now,sessions}:{component:Component;device:Device;context:Context;api:Api;refresh:Refresh;now:number;sessions:SessionSnapshot[]}){
 ...
 const reread:Reread=async pick=>{
  ...
  if(!latest||latest.error)return {blocked:'BUNNY couldn’t read the device’s current state'};
 ...
 const reapplyCommand=useCommand(api,path+'/commands',refresh,snapshot);
 ...
 const startMonitor=pixoo(integr)&&pixooMode?.mode==='monitor'?<div className="switch">{!pixooMode.participating&&<><div className="actions"><button type="button" disabled={!!startReason||monitorCommand.busy||monitorCommand.locked} onClick={()=>void monitorCommand.run('Start Monitor',()=>reread((latest,a)=>{const i=latest.integration,s=latest.snapshot;return a.disabled??(!pixoo(i)||i.configuration.mode!=='monitor'?'Pixoo is no longer set to Monitor':i.participating?'Monitor is already showing':i.pendingMode?'a mode change is pending':s?.state.desired.power.status==='known'&&!s.state.desired.power.value?screenOff:{request:pixooRequest(i,component,{operation:'mode',mode:'monitor'})});}))}>Start Monitor</button>{monitorCommand.locked&&<button type="button" className="secondary" onClick={monitorCommand.unlock}>Reload current values</button>}</div><p className="hint">Pixoo is set to Monitor but isn’t showing agent status. Start Monitor sends one Monitor mode command and doesn’t start playback.{startReason?` Unavailable: ${startReason}.`:''}</p></>}<p role="status" data-tone={monitorCommand.tone}>{monitorCommand.status}</p></div>:undefined;
 ...
 {pixoo(integr)&&<EditForm title="Mode" source={integr} revision={`${integr.configurationRevision}:${integr.generation}`} initial={{mode:integr.configuration.mode}} disabled={disabled} api={api} path={path+'/integration/commands'} build={(s,v)=>pixooRequest(s,component,{operation:'mode',mode:v.mode})} refresh={refresh} prepare={()=>reread((latest,a)=>a.disabled??(pixoo(latest.integration)?{source:latest.integration,revision:`${latest.integration.configurationRevision}:${latest.integration.generation}`}:'No integration snapshot'))} extra={startMonitor}>{(v,c)=><><Select label="Device mode" value={v.mode} onChange={x=>c('mode',x)} options={integr.capabilities.modes.map(m=>({value:m,label:title(m)}))}/><p className="hint">Monitor presents agent activity. Media opts out; collection continues. This does not start playback. Pending mode: {integr.pendingMode?title(integr.pendingMode):'none'}.</p></>}</EditForm>}
...
function GeneralControls({component,snapshot,integration,reasons,api,path,refresh,reread,mediaExtra,sceneExtra}:{component:Component;snapshot:Snapshot;integration:Nano|Pixoo|undefined;reasons:GeneralReasons;api:Api;path:string;refresh:Refresh;reread:Reread;mediaExtra?:React.ReactNode;sceneExtra?:React.ReactNode}){
 ...
 return <>
 <EditForm title={power} source={snapshot} revision={revision} initial={{on:snapshot.state.desired.power.status==='known'?(snapshot.state.desired.power.value?'on':'off'):'on'}} disabled={reasons.power} api={api} path={path} build={(s,v)=>makeCommand(s,{kind:'power.set',on:v.on==='on'})} refresh={refresh} prepare={prepare('power')}>{(v,c)=><><Select label={power} value={v.on} onChange={x=>c('on',x)} options={[{value:'on',label:'On'},{value:'off',label:'Off'}]}/><p className="hint">{pixel?'Screen off pauses playback; screen on does not resume it. Power works in Monitor and Media and does not change the mode.':wall?'Power works in Work, Quiet and Free and does not change the mode. While off, the wall keeps tracking tasks and writes nothing until the next explicit mode command.':'Power does not change the device mode.'}</p></>}</EditForm>
 ...
 <MediaControls snapshot={snapshot} reason={reasons.media} api={api} path={path} refresh={refresh} reread={reread} extra={mediaExtra}/>
 <SceneControls snapshot={snapshot} integration={integration} reason={reasons.scenes} api={api} path={path} refresh={refresh} reread={reread} extra={sceneExtra}/>
 </>;
}
...
function NanoMappings({integration:s,disabled,api,path,refresh,reread}:{integration:Nano;disabled?:string;api:Api;path:string;refresh:Refresh;reread:Reread}){
 ...
 const base={source:s,revision:s.revision,api,path:path+'/integration/commands',refresh,prepare:()=>reread((latest,a)=>a.disabled??(nano(latest.integration)?{source:latest.integration,revision:latest.integration.revision}:'No integration snapshot'))};
