...
import {age,nano,pixoo,title,listed,Badge,Facts,EditForm,Select,options,useCommandLifecycle,deviceControls,hasModeControl,undeclaredCapabilities,lightingNote,ModeCard,PowerCard,BrightnessCard,MediaCard,SceneCard,LightingCards,NanoAssignments,PixooMonitor,type Monitor,type Nano,type Pixoo,type Device,type Refresh,type DeviceControls} from './controls';
...
function PlaybackView({api,sourceId,control,now}:{api:Api;sourceId:string;control:boolean;now:number}){
 ...
 <div className="cards"><div className="edit" role="group" aria-labelledby={heading}><h3 id={heading}>Playback controls</h3>
 ...
 <p role="status" data-tone={command.tone}>{command.status}</p></div></div>
 </>;
}
...
function Widget({id,size,title,link,children}:{id:string;size:WidgetSize;title?:string;link?:{route:Route;label:string};children:React.ReactNode}){
 const heading=useId();
 return <article className="widget" data-widget={id} data-size={size} aria-labelledby={heading}><header className="widget-head"><h2 id={heading}>{title??widgetDefinition(id)?.name??id}</h2>{link&&<NavLink route={link.route}>{link.label}</NavLink>}</header>{children}</article>;
}
