...
function SessionRow({session:s,monitor,context,api,refresh,stale,elapsed}:{session:SessionSnapshot;monitor:Monitor;context:Context;api:Api;refresh:()=>Promise<void>;stale:boolean;elapsed:number}){
 return <article className="session"><div className="session-head"><Badge warning={s.freshness!=='current'||stale}>{stale?'Stale connection':s.freshness}</Badge><div className="session-id"><h3>{s.label??s.title?.value??s.identity.sessionId}</h3><p className="muted">{s.project&&<><span>{s.project}</span> · </>}{s.identity.provider} · {s.identity.client} · {s.identity.hostId} / {s.identity.sourceId}</p></div></div>
 ...
 ]}/><EditForm title="Label" className="inline" source={monitor} initial={{label:s.label??''}} disabled={!context.control?'Read-only credential':stale?'Snapshot is stale':undefined} api={api} path="/api/monitor/v1/commands" device={false} build={(m,v)=>({operation:'label',requestId:m.nextRequestId,identity:s.identity,label:v.label||null})} refresh={refresh}>{f=><TextField label="Chosen label" name="label" field={f} maxLength={160} pattern=".{0,80}" title="Use at most 80 characters." placeholder="Choose a label"/>}</EditForm>
...
function AttentionWidget({sessions,size}:{sessions:SessionSnapshot[];size:WidgetSize}){
 ...
 return <Widget id="attention" size={size}>{waiting.length?<ul className="plain">{waiting.map(s=><li key={key(s)}><span className="attention">{s.attention.map(a=>a.kind==='question'?'Question':title(a.kind)).join(', ')}</span> · {s.label??s.title?.value??s.identity.sessionId}</li>)}</ul>:<p className="hint">No session is waiting on a question or an approval.</p>}</Widget>;
}
