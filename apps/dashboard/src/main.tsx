...
function App(){
 ...
 return api?<Dashboard api={api} disconnect={disconnect}/>:<main className="login"><p className="eyebrow">B.U.N.N.Y. / LOCAL INTEGRATION</p><h1>Your workspace.<br/>One clear view.</h1><p>{launching?'Connecting to the local Hub…':'Open B.U.N.N.Y. with the Hub launcher.'}</p>{launchError&&<p role="alert">That launch expired or failed. Run the launcher again.</p>}<p className="hint">The launcher opens this page and connects automatically. After a reload, run it again.</p><details><summary>Use a separately provisioned access token</summary><form onSubmit={e=>{e.preventDefault();if(/^[A-Za-z0-9_-]{43}$/.test(token)){setApi(new Api(token));setToken('');}}}><label>Hub browser access token<input type="password" autoComplete="off" required pattern="[A-Za-z0-9_-]{43}" value={token} onChange={e=>setToken(e.target.value)}/></label><button>Connect</button></form><p className="hint">Never use a native controller token. Browser access stays in page memory and clears on disconnect or reload.</p></details></main>;
}
