...
try {
 ...
 await page.getByLabel('Layout style').selectOption('project');await page.getByRole('button',{name:'Apply integration settings',exact:true}).click();await page.getByText('applied. Physical result is not confirmed.',{exact:true}).waitFor();
 assert.equal(f.writes[1].command.command.style,'project');
 await page.getByLabel('Element project').selectOption(f.nano.projects[0].id);await page.getByRole('button',{name:'Apply element mapping',exact:true}).click();await page.waitForFunction(()=>Array.from(document.querySelectorAll('[role=status]')).filter(x=>x.textContent==='applied. Physical result is not confirmed.').length>=2);assert.equal(f.nano.elements[0].projectId,f.nano.projects[0].id);
 await page.getByLabel('Task project').selectOption(f.nano.projects[0].id);await page.getByRole('button',{name:'Apply task mapping',exact:true}).click();await page.waitForFunction(()=>Array.from(document.querySelectorAll('[role=status]')).filter(x=>x.textContent==='applied. Physical result is not confirmed.').length>=3);assert.equal(f.nano.tasks[0].overrideProjectId,f.nano.projects[0].id);
 await page.getByLabel('Color', {exact:true}).fill('#112233');await page.getByRole('button',{name:'Apply project color',exact:true}).click();await page.waitForFunction(()=>Array.from(document.querySelectorAll('[role=status]')).filter(x=>x.textContent==='applied. Physical result is not confirmed.').length>=4);assert.equal(f.nano.projects[0].color,'#112233');
...
}finally{await page.close();await browser.close();await f.close();}
