...
try {
 ...
 await page.getByRole('button',{name:'Disconnect',exact:true}).click();await page.getByText('Use a separately provisioned access token').click();await page.getByLabel('Hub browser access token').fill(f.reader);await page.getByRole('button',{name:'Connect',exact:true}).click();await page.getByRole('heading',{name:'My deliberate label',exact:true}).waitFor();await page.getByRole('link',{name:'wall nanoleaf',exact:true}).click();await page.getByText('Unavailable: Your credential is read-only',{exact:true}).first().waitFor();assert.equal(await page.getByLabel('Device mode').first().isDisabled(),true);
