...
try {
 ...
 await page.getByLabel('Color project').selectOption(f.nano.projects[1].id);assert.equal(await page.locator('input[type=color]:visible').inputValue(),'#ff0000');assert.equal(await page.getByRole('button',{name:'Apply project color',exact:true}).isDisabled(),true);await page.locator('input[type=color]:visible').fill('#112233');await page.getByRole('button',{name:'Apply project color',exact:true}).click();await page.waitForFunction(()=>Array.from(document.querySelectorAll('[role=status]')).filter(x=>x.textContent==='applied. Physical result is not confirmed.').length>=4);assert.equal(f.nano.projects[1].color,'#112233');
