...
try {
 ...
 assert.equal(await page.getByRole('heading',{name:'Home',exact:true}).count(),0,'the home is not shown for the activity alias');
 await page.getByRole('link',{name:'connections sensor',exact:true}).click();await page.locator('section:visible .section-heading h2',{hasText:/^connections$/}).waitFor();
 assert.equal(await page.getByRole('heading',{name:'Connections',exact:true}).count(),0,'the Connections page is not shown for the connections alias');
 ...
 await page.getByLabel('Layout style').selectOption('project');await applyNano('Apply integration settings');await page.getByText('Saved. B.U.N.N.Y. can’t see the device, so check it to confirm.',{exact:true}).waitFor();
 ...
 await page.getByRole('link',{name:'wall nanoleaf',exact:true}).click();await page.getByLabel('Device mode').first().selectOption('Free');const concurrent={apiVersion:'1.0',controllerId:f.states.wall.identity.controllerId,deviceId:'wall',requestId:f.states.wall.nextRequestId,expectedConfigurationRevision:f.states.wall.configurationRevision,expectedGeneration:f.states.wall.generation,command:{kind:'mode.set',mode:'Work'}};const concurrentResult=await fetch(f.hub.url+'/api/controllers/v1/wall/commands',{method:'POST',headers:f.headers,body:JSON.stringify(concurrent)});assert.equal(concurrentResult.status,200);
 await page.waitForTimeout(5500);await page.locator('section:visible').getByText('Another client changed this setting while you were editing.',{exact:false}).first().waitFor();assert.equal(await page.getByLabel('Device mode').first().inputValue(),'Free');assert.equal(await page.getByRole('button',{name:'Apply mode',exact:true}).first().isDisabled(),true);
 f.setOffline(true);await page.waitForTimeout(5500);await page.getByRole('link',{name:'pixel pixoo',exact:true}).click();await page.getByText('Stale / unavailable',{exact:true}).waitFor();
 f.setOffline(false);await page.waitForTimeout(5500);
 await page.getByRole('link',{name:'wall nanoleaf',exact:true}).click();await page.locator('section:visible').getByRole('button',{name:'Discard my edit'}).first().click();await page.getByLabel('Device mode').first().selectOption('Quiet');f.setUncertain(true);const submitted=f.writes.length;await page.getByRole('button',{name:'Apply mode',exact:true}).first().click();await page.getByText('Result unknown: this may have reached the device',{exact:false}).waitFor();await page.waitForTimeout(5500);assert.equal(f.writes.length,submitted+1,'uncertain command is not retried');f.setUncertain(false);
 ...
 await page.getByRole('button',{name:'Disconnect',exact:true}).click();await page.getByText('Use a separately provisioned access token').click();await page.getByLabel('Hub browser access token').fill(f.reader);await page.getByRole('button',{name:'Connect',exact:true}).click();await page.getByRole('heading',{name:'My deliberate label',exact:true}).waitFor();await page.getByRole('link',{name:'wall nanoleaf',exact:true}).click();await page.getByText('Unavailable: Your credential is read-only',{exact:true}).first().waitFor();assert.equal(await page.getByRole('button',{name:'Apply mode',exact:true}).first().isDisabled(),true);
