...
/** `geometry`: 'layout' serves the layouts above, 'none' serves an explicit empty result for the wall, 'older' answers like an owner that predates the route. `panels` adds the NL22 Panels as a second Nanoleaf device whose integration snapshot is read-only (codex-nanoleaf#113). */
export async function fixture({empty=false,playback=false,panels=false,geometry='layout'}={}){
 ...
 // The Panels keep the exact snapshot key set but mark the four configuration operations unsupported and list no elements; the hub rejects that until Hub #323.
 const panelsExt={...structuredClone(nano),identity:identityOf('panels'),projects:[],tasks:[],elements:[],scenes:[],capabilities:Object.fromEntries(['settings.set','elements.assign','task.assign','project.color','mode.set'].map(k=>[k,{supported:k==='mode.set',scope:'control',...(k==='mode.set'?{route:'/controller/v1/commands'}:{})}]))};
 ...
 const states={wall:structuredClone(template),pixel:structuredClone(template),...(panels?{panels:structuredClone(template)}:{})};
 for(const [id,s] of Object.entries(states)){s.identity={controllerId:id==='pixel'?'pixel-controller':'wall-controller',deviceId:id,sourceId:id,controllerEpoch:'epoch'};s.state.desired.mode={status:'known',value:id==='wall'?'Work':'Media'};s.state.externalControl={status:'unknown'};s.state.observation={status:'unknown'};s.state.lastSuccessfulSend={status:'unknown'};s.state.lastOutcome={status:'unknown'};s.state.pending=[];}
 ...
 if(states.panels){states.panels.capabilities=structuredClone(states.wall.capabilities);states.panels.capabilities.scenes={supported:false};states.panels.state.desired.mode={status:'known',value:'Work'};states.panels.state.desired.power={status:'unknown'};states.panels.state.desired.brightness={status:'unknown'};}
 ...
 for(const id of Object.keys(states)){
  const server=createServer(async(req,res)=>{
   ...
   const integration=req.url.includes('integration');const state=states[id],ext=id==='wall'?nano:id==='panels'?panelsExt:pixoo;
   ...
   if(id==='panels'){send(422,{apiVersion:nano.apiVersion,requestId:command.requestId,outcome:'failed',priorEffects:'none',physicalOutcome:'unknown',failure:{code:'unsupported-capability'}});return;}
   if(id==='wall'){
