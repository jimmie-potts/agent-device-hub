 ...
 for(const id of ['wall','pixel']){
  const server=createServer(async(req,res)=>{
   ...
   if(!integration){if(!validate('request',command)){send(400,{failure:{code:'invalid-request'}});return;}
    ...
    // Like Nanoleaf main 08b6b83, any mode command ends power and brightness overrides; the same mode with nothing to reapply is cancelled with no effects.
    const overridden=state.state.desired.power.status==='known'||state.state.desired.brightness.status==='known';
    if(!failure&&id==='wall'&&command.command.kind==='mode.set'&&state.state.desired.mode.value===command.command.mode&&!overridden){state.nextRequestId.sequence++;const cancelled={...receipt,outcome:'cancelled'};state.state.lastOutcome={status:'known',receipt:cancelled};send(200,cancelled);return;}
