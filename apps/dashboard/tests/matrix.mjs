...
try {
 ...
 await scenario('slow device serializes explicit commands while another device remains responsive',async(f,page)=>{
  ...
  // The page's 5 s poll reads wall alongside pixel, and wall's one-slot client answers an overlapping read with capacity (429). Retry only that, timed from the first attempt, so wall must still answer long before the 800 ms pixel read ends.
