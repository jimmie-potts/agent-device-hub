...
Credentials need the source ID in `devices`: `read` scope for snapshots and `control` scope for commands. See [Credentials](#credentials) to create one. Launcher browser sessions receive both on the configured source, and B.U.N.N.Y. shows it under Music ([#37](https://github.com/jimmie-potts/agent-device-hub/issues/37)). The MCP playback tools are described under [Optional local MCP](#optional-local-mcp).
