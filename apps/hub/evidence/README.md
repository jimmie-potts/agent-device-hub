...
Final review found that a supplied consumer URL could point back to the host without Pixoo running. The repaired activation API requires a live supervised Pixoo capability, binds it to the exact selected configuration loaded at startup, derives endpoint/credential internally and rechecks liveness and unchanged files. The missing-facade regression failed before repair and passes afterward; copied/stopped handles and mismatched configuration also reject. The pinned real Pixoo cutover check was rerun after this change. The earlier API receipt remains historical and source-hashed; migration-only changes do not make it a new measurement.
