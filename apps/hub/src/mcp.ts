...
export function createHubMcp(options:Options):McpHandler {
 ...
 const host:Record<string,ServiceExtension>={
  sessions:extension(HOST_SERVICE,'read',readDescription('Read observed agent sessions, optionally matching q against the chosen label or session ID and filtering by provider. Returns the qualified snapshot, matching identities and the next request ID for hub_label and hub_acknowledge.'),shape({q:{type:'string',maxLength:120},provider:{enum:['codex','claude']}},[]),(args,p)=>options.sessions(p,args.q as string|undefined,args.provider as string|undefined)),
