...
export function createHubMcp(options:Options):McpHandler {
 ...
 const host:Record<string,ServiceExtension>={
  sessions:extension(HOST_SERVICE,'read',readDescription('Read observed agent sessions, optionally filtering by provider and by q, a case-insensitive match against each session\'s label, title, project or session ID. Returns the qualified snapshot, matching identities and the next request ID for hub_label, hub_acknowledge and hub_recover_approval.'),shape({q:{type:'string',maxLength:120},provider:{enum:['codex','claude']}},[]),(args,p)=>options.sessions(p,args.q as string|undefined,args.provider as string|undefined)),
  ...
  label:extension(HOST_SERVICE,'control',writeDescription('Set the user-chosen label for one session identity, or pass null to remove it. Use the next request ID from hub_sessions as request_id.'),shape({request_id:requestString,identity,label:{anyOf:[{type:'string',minLength:1,maxLength:80},{type:'null'}]}}),(args,p)=>options.command(p,{operation:'label',requestId:args.request_id,identity:args.identity,label:args.label})),
