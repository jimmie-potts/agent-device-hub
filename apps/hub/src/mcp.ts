...
export function createHubMcp(options:Options):McpHandler {
 ...
 for(const [alias,client] of options.clients){
  ...
  if(kind==='lifx'){
   extensions.lighting_status=extension(alias,'read',readDescription('Read this LIFX bulb\'s lighting snapshot: its controller v1 snapshot, whether color and white temperature are qualified, pending lighting requests, and the last observed color in LIFX wire units with its age. Its request identity and revision/generation guards serve color_set and temperature_set.'),shape({}),()=>client.lightingSnapshot());
   ...
   lighting('temperature_set','Set this LIFX bulb to white at a color temperature in kelvin.',{kelvin:{type:'integer',minimum:1500,maximum:9000}},a=>({kind:'lifx.temperature.set',kelvin:a.kelvin}));
