...
async function readConfiguration(path:string):Promise<HubOptions> {
  ...
  if (!object(value) || !exact(value,['directory','ownerId','consumers','credentials','controllers','port']) || typeof value.directory !== 'string' || typeof value.ownerId !== 'string') throw new Error('invalid-configuration');
