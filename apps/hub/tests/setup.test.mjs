...
import {planSetup,applySetup,removeSetup,inspectSetup} from '../dist/setup.js';
...
test('plan is read-only; idempotent setup and surgical removal preserve latest unrelated settings',async t=>{
 ...
 await removeSetup(input.directory,access);assert.equal(access.active.size,0);const final=await read(input.target);assert.equal(final.newPreference,'KEEP');assert.deepEqual(final.hooks.Stop,[{hooks:[{type:'command',command:'new-user-hook'}]}]);assert.equal(final.trust,'unchanged');
});
test('stale plans, changed owned entries and interrupted credential operations fail safely',async t=>{
 ...
 const saved=await readFile(input.target,'utf8'),edited=JSON.parse(saved);edited.hooks.Stop[0].hooks[0].timeout=10;await write(input.target,edited);await assert.rejects(removeSetup(input.directory,access),/owned-entry/);assert.equal(access.active.size,1);
 await writeFile(input.target,saved,{mode:0o600});await assert.rejects(removeSetup(input.directory,{...access,revoke:async()=>{throw new Error('offline');}}),/offline/);assert.equal((await inspectSetup(input.directory)).state,'removing');await removeSetup(input.directory,access);await removeSetup(input.directory,access);assert.equal(access.active.size,0);
});
