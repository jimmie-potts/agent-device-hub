...
- **LIFX.** The host owns one `LifxController` for the configured bulbs and holds one writer lease per bulb address under `~/.local/state/agent-device-hub/lifx/`. It never polls, discovers or paints a bulb on its own. A bulb changes only for an explicit admitted command, so a manual change in the LIFX app stays until the next one. Automatic LIFX status belongs to [#20](https://github.com/jimmie-potts/agent-device-hub/issues/20) and [#22](https://github.com/jimmie-potts/agent-device-hub/issues/22).
