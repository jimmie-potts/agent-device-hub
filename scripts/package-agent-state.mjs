...
const NAME='@jimmie-potts/agent-state',VERSION='2.0.1';
...
try{
  ...
  if(process.argv.includes('--test')){
    ...
    assert.equal(run(process.execPath,['--input-type=module','-e',
      'import {VERSION,FORMAT_VERSION,validateSnapshot} from "@jimmie-potts/agent-state"; import {normalizeHook} from "@jimmie-potts/agent-state/providers"; if(VERSION!=="2.0.1"||FORMAT_VERSION!=="1.0"||validateSnapshot({}).ok||typeof normalizeHook!=="function")process.exit(1);'],consumer),'');
