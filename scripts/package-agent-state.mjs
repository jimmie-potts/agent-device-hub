...
const NAME='@jimmie-potts/agent-state',VERSION='3.3.0';
...
const scratch=await mkdtemp(join(scratchRoot,'state-'));
try{
  ...
  await writeFile(join(stage,'manifest.json'),JSON.stringify({artifact:NAME,version:VERSION,apiVersion:'1.0',snapshotVersions:['1.0','1.1','1.2'],formatVersion:'2.1',schemaDraft:'2020-12',fixtureFormat:1,
  ...
  if(process.argv.includes('--test')){
    ...
    assert.equal(run(process.execPath,['--input-type=module','-e',
      'import {VERSION,FORMAT_VERSION,validateSnapshot} from "@jimmie-potts/agent-state"; import {normalizeHook} from "@jimmie-potts/agent-state/providers"; if(VERSION!=="3.3.0"||FORMAT_VERSION!=="2.1"||validateSnapshot({}).ok||typeof normalizeHook!=="function")process.exit(1);'],consumer),'');
