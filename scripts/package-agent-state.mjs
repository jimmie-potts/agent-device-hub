...
const NAME='@jimmie-potts/agent-state',VERSION='3.1.0';
...
try{
  ...
  if(process.argv.includes('--test')){
    ...
    assert.equal(run(process.execPath,['--input-type=module','-e',
      'import {VERSION,FORMAT_VERSION,validateSnapshot} from "@jimmie-potts/agent-state"; import {normalizeHook} from "@jimmie-potts/agent-state/providers"; if(VERSION!=="3.1.0"||FORMAT_VERSION!=="2.0"||validateSnapshot({}).ok||typeof normalizeHook!=="function")process.exit(1);'],consumer),'');
