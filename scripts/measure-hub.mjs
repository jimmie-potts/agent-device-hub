...
for(const tasks of [1,10,50])for(let repeat=1;repeat<=3;repeat++){
 const directory=await mkdtemp(join(tmpdir(),'hub-measure-'));let child,timer;
 ...
 try{
  ...
  const exited=once(child,'exit');timer=setTimeout(()=>child.kill('SIGKILL'),120000);
 ...
 finally{clearTimeout(timer);if(child?.exitCode===null){child.kill('SIGKILL');await once(child,'exit').catch(()=>{});}await rm(directory,{recursive:true,force:true});}
