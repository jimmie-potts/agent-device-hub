...
const scratch=await mkdtemp(join(tmpdir(),'hub-contract-package-'));
try{
 ...
 if(process.argv.includes('--test')){
  ...
  await writeFile(join(consumer,'package.json'),JSON.stringify({name:'isolated-contract-consumer',private:true,type:'module'}));
  assert.equal(sha256(await readFile(archive)),checksum);
  npm(['install','--ignore-scripts','--no-audit','--no-fund',archive],consumer);
  const installed=join(consumer,'node_modules/@jimmie-potts/device-contracts');
