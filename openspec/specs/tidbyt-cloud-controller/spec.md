# tidbyt-cloud-controller Specification

## Purpose
Define the fake-testable Tidbyt cloud controller from [Hub #16](https://github.com/jimmie-potts/agent-device-hub/issues/16): a pure 64×32 WebP renderer, a background-push cloud connection with private credentials, one serialized display queue on controller v1 semantics, and outcome reporting that keeps transmission separate from unknown visible-device evidence.
