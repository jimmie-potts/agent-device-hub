...
#### Scenario: Uncertain sessions still paint their reported state
- **WHEN** every shown root session has `uncertain` freshness, including a finished turn idle for more than five minutes, and the bulb's mode is Work
- **THEN** the publisher paints the highest state those sessions report, and paints again at the next transition

#### Scenario: An unavailable feed paints nothing
- **WHEN** the feed read fails or the collector is not running
- **THEN** the publisher paints nothing and the bulb keeps its last appearance
