...
No durable format change. Opening an existing 2.0 or 1.0 store with Agent State 3.2.0 settles stored ended records once, preserving clocks. Compatibility is one-way: the 3.0 and 3.1 validators reject Claude and CLI retirement guards, so those owners fail closed (`invalid-storage`) on a store or export holding one, however old. A guard expires 24 hours after its retirement, but only a running or reopened 3.2.0 owner prunes it. Rollback to those versions follows the existing owner handoff rules after a 3.2.0 owner has pruned the guards, or through an explicitly reconciled export; an older owner would again retain Claude/CLI ends until expiry. Source checks use disposable stores; no installed store is opened.
