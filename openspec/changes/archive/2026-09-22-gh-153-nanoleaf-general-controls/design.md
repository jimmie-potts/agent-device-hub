...
- A submitted control keeps keyboard focus. Disabling the focused control while a command runs would drop focus to the document, so the shared draft form and one-click hook capture the focused control before disabling and, once the command settles, return focus to it or, when it is locked or unmounted, to the first enabled control of the same group. This came from the accessibility review of the candidate and also covers the Pixoo controls that share the components.
