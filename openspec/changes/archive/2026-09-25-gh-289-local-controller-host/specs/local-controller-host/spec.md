...
One loopback process that owns the in-process Tidbyt and LIFX controllers and serves controller v1, plus the LIFX lighting profile, so the hub can route to them like any other controller service.
...
The host SHALL be the only writer for its devices while it runs. It SHALL start the Tidbyt runner in-process, which holds the existing cloud-device lease and runs the status and optional now-playing publishers through the one Tidbyt queue. It SHALL hold one OS-released lease per configured bulb address for its lifetime. It SHALL NOT poll bulbs, send status, or change a bulb except for an explicit admitted command, so a manual change in the LIFX app stays until the next explicit command.
...
- **THEN** the host sends no bulb traffic, and its snapshot keeps its last observation with its age instead of claiming the manual state
