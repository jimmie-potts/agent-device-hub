...
3. **Liveness re-check at ticket use.** Authorization happens before request bodies are read and before MCP handlers await. Both `command` and `sessions` now confirm that the principal is still a current configured credential or live browser session before touching its ledger; otherwise they return `unauthenticated`. This prevents a retired principal from recreating a ledger. `authenticate` now prunes expired sessions on every call, so expiry takes effect even when only configured credentials are active.
