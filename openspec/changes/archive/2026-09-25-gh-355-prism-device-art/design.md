...
- **Geometry is read once per component.** After the first successful snapshot the page reads the geometry route once; a failed read is retried on the next poll; a 422 from an owner without the route is final for the session. Alternative: read on every poll; rejected because a second read per poll collides with the one-slot device client (#354) and layouts rarely change. Reopening the page rereads it.
