...
- Power, brightness and playlist selection reuse the existing draft form: the draft pins the observed snapshot, a changed revision blocks submission, and uncertain or partial results lock the form. Playback actions are one-click commands that use the latest observed snapshot's ticket, revision and generation at activation time; a stale revision surfaces as the controller's typed `revision-conflict` failure. Alternative: drafts for actions; rejected because an action has no editable value.
