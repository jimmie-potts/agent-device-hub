...
- A child session waiting for approval makes its root `RUN`, not `ASK`, because the owner's child counts cover activity only. Showing child attention would need a child row or a new owner count.
- After repeated failures the backoff can delay a changed frame by up to 10 minutes. The cadence decision sets only a maximum rate, so this is accepted.
