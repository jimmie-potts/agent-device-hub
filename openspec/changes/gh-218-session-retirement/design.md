...
5. The host supplies bounded archive evidence at new-record admission through the existing Codex-home configuration. Inspect only matching archive filenames for the candidate or its known ancestors, including an absent topmost parent, never file contents. Walk each known identity once and scan the archive once within the same deadline. Missing evidence fails open. A 200 ms deadline and finite directory-entry count bound the lookup; retirement never invokes it. No background archive polling or retained stale archive result.
