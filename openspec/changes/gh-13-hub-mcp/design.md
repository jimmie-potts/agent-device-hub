...
- Expose typed native integration edits, mode changes and supported common media commands. No reusable Pixoo player/catalog service is registered by the standalone host; do not import controller source or invent a browser-token bridge. Native media operations use the existing common controller route.
